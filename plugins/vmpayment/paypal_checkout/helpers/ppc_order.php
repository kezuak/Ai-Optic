...
class PayPalOrder {
	...
	static function createExperienceContext($plugin, $cart){
		...
		if($cart->_dataValidated and $view=='cart'){
			$exp_contxt->user_action = 'PAY_NOW';
		} else {
			$exp_contxt->user_action = 'CONTINUE';
		}

		$exp_contxt->return_url = JURI::root(false).'/index.php?com_virtuemart&view=cart&layout=orderdone';
		$exp_contxt->cancel_url = JURI::root(false).'/index.php?com_virtuemart&view=cart';
...
vmdebug('my discount is '.round((float)$totalInPaymentCurrency['value'],2).' - '.round((float)$shipmentValue,2).' - '.round($item_total->value,2).' - '.round((float)$cart->cartPrices['taxAmount'] ,2).' = '.$discount->value);
		...
		$purchase_units = array('amount'    => $amount,
			'items'     => $items,
		...
		if(isset($body->id)){
			...
			vmdebug('createOrder returning OrderId '.$body->id);
			VmEcho::$logDebug = 0;
			return $body->id;
		} else {
			return false;
	 ...
	 * @param $order
	 */
	static function createOrder($plugin, &$cart, $order){
...
vmdebug('createOrder $totalInPaymentCurrency = '.$totalInPaymentCurrency['value'].'  discount '.round((float)$order['details']['BT']->order_discount,2));
		...
		$purchase_unit = array('amount'    => $amount,
