	...
	function plgVmOnProductDisplayPayment($product, &$productDisplay) {
		...
		if($product->orderable) {
			...
			foreach ($this->methods as $this->_currentMethod) {
				...
				if(empty($this->_currentMethod->button_in_product) /*or (empty($this->_currentMethod->button_for_guests) and empty($user->id))*/ ){
					continue;
				}

				$this->getPaymentCurrency($this->_currentMethod, true);
				$this->addPaypalJSSDK('product-details');
...
/*	function plgVmOnCheckoutAdvertise($cart, &$payment_advertise) {
	...
	protected function renderPluginName ($method) {
		...
		return '<span class="' . $this->_type . '_name">' . $text . '</span>';
	...
	function getCartHash(){
		...
		$merchantId = $this->_currentMethod->{$sand.'paypal_merchant_id'};

		$h = hash('md5', vmJsApi::safe_json_encode($cart->cartProductsData).$cart->virtuemart_paymentmethod_id.$merchantId);
		vmdebug('getCartHash ',$h);
		return $h;
	}
	...
	function plgVmOnSelfCallFE($type, $name, &$render) {
		...
		vmdebug('PayPal I am in plgVmOnSelfCallFE '.$task);
		if ($task == 'checkout'){
			...
			$this->storePayPalData($cart->virtuemart_paymentmethod_id, $this->_currentMethod);
	...
	function plgVmOnSelfCallBE($type, $name, &$render) {
		...
		//VmEcho::$echoDebug = 1;
		vmdebug('PayPal I am in plgVmOnSelfCallBE ' . $task);
	...
	function plgVmConfirmedOrder($cart, $order){
		...
		if($this->_currentMethod->paypal_products == 'pui') {
			...
			self::$vmPPOrderId = PayPalOrder::createOrder($this, $cart, $order);
			//Does not work? why?
			...
			if(!empty(self::$vmPPOrderId)){
				...
				if(empty($body)){
					if($this->_currentMethod->paypal_intent == 'capture'){
						PayPalOrder::captureOrder($this, self::$vmPPOrderId);
					} else if($this->_currentMethod->paypal_intent == 'authorize'){
						PayPalOrder::authorizeOrder($this, self::$vmPPOrderId);
					}
	...
	function storePayPalData($orderDetails, $method){

		if(!is_object($orderDetails)){
			$orderModel = VmModel::getModel('orders');
			$order = $orderModel->getOrder($orderDetails);
...
//if(empty(self::$PPResult->body))
	...
	//vmTrace('storePayPalData ');
		...
		/*if(!empty(self::$PPResult->resource_type) ){
			self::$PPResult->resource_type =
		}*/
		/*self::$PPResult->payment_name = null;

		/*self::$PPResult->payment_order_total = $order['details']['BT']->virtuemart_order_id;
		self::$PPResult->payment_currency = $order['details']['BT']->virtuemart_order_id;
		self::$PPResult->email_currency = $order['details']['BT']->virtuemart_order_id;
		self::$PPResult->cost_per_transaction = null;
		self::$PPResult->cost_percent_total = null;
		self::$PPResult->tax_id = null;*/
		//self::$PPResult->body = null;
		$data = get_object_vars(self::$PPResult);
		$data['id'] = 0;
		vmTrace('Storing my pp result for orderid '.self::$PPResult->virtuemart_order_id, FALSE, 3);
	 ...
	 * @return bool
	 */
	function addPaypalJSSDK($pageType){
		...
		if(!isset($selected)) {
			$selected = ($cart->virtuemart_paymentmethod_id == $this->_currentMethod->virtuemart_paymentmethod_id)? 1:0;
		...
		$nvp['client-id'] = $this->_currentMethod->{$sandbox.'client_id'};

		$currencyM = VmModel::getModel('currency')->getCurrency($this->_currentMethod->payment_currency);
		$nvp['currency'] = $currencyM->currency_code_3;

		if(!empty($this->_currentMethod->debug)){
			$nvp['debug'] = $this->_currentMethod->debug;
		}
		...
		if(!empty($cart->BT['virtuemart_country_id']) and !empty($this->_currentMethod->sandbox)){
		...
		if(!empty($this->_currentMethod->disable_funding)){
			if(is_array($this->_currentMethod->disable_funding)){
				$nvp['disable-funding'] = implode(',', $this->_currentMethod->disable_funding);
			} else if(!empty($this->_currentMethod->disable_funding)){
				$nvp['disable-funding'] = $this->_currentMethod->disable_funding;
			}
		}

		if(!empty($this->_currentMethod->enable_funding)){
			if(is_array($this->_currentMethod->enable_funding)){
				$nvp['enable-funding'] = implode(',', $this->_currentMethod->enable_funding);
			} else if(!empty($this->_currentMethod->enable_funding)){
				$nvp['enable-funding'] = $this->_currentMethod->enable_funding;
			}
		...
		//if( count($products) == 1 and isset($products['buttons']) ){
			$nvp['intent'] = strtolower($this->_currentMethod->paypal_intent);
		//}

		$nvp['integration-date'] = '2023-08-08';

		/*if(!empty($this->_currentMethod->merchant_id)){
			$nvp['merchant_id'] = $this->_currentMethod->merchant_id;
		}*/
		...
		$this->addJSScriptWithPayPalHeader($pageType, $nvpUrl, $client_token);
		...
		if( (isset($products['buttons']) and empty($vmPPOrderId)) or isset($products['hosted-fields']) or isset($products['pui'])){
			...
			if( !$cart->_dataValidated){
				...
				if(strtolower($this->_currentMethod->paypal_intent) == 'capture'){
					$task = 'captureOrder';
				} else if(strtolower($this->_currentMethod->paypal_intent) == 'authorize'){
					$task = 'authorizeOrder';
				} else {
					$task = 'createOrder';
				}
	...
	function plgVmOnShowOrderBEPayment($virtuemart_order_id, $payment_method_id) {
		...
		foreach($payments as $entr){
			...
			if(isset($entr->body->status)){
				$html .= $this->getHtmlRowBE ('VMPAYMENT_PAYPAL_PAYMENT_STATUS', $entr->body->status);
			}
...
//vmdebug('plgVmSetOnTablePluginParamsPayment'.$id,$this->_currentMethod);
		...
		$methods = $this->getPluginMethods($this->_currentMethod->virtuemart_vendor_id, false);
